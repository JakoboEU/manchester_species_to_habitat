...
```{r}
data = read_csv('./included_species.csv')
data
```

```{r}
avonet = read_csv('./AVONET2_eBird.csv')
avonet
```
...
```
```{r}
birdlife_data = read_excel('./birdlife_datazone_habitats.xlsx')
birdlife_data
```
...
```{r}
plant_data = read_excel('./PLANTATT_19_Nov_08.xls')
...
```{r}
insect_data = read_excel('./wildlifetrust_habitats.xlsx')
insect_data
```
...
```{r}
write_csv(species_to_habitat, 'species_habitats.csv')
```
