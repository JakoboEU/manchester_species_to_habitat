...
```
Call:
glm(formula = richness ~ herb + mat_tr + shrb + herb:mat_tr + 
    herb:shrb + mat_tr:shrb + herb:mat_tr:shrb + 1, family = gaussian, 
    data = insect_data)

Coefficients:
                   Estimate Std. Error t value Pr(>|t|)    
(Intercept)       4.423e-01  1.113e-01   3.973  8.4e-05 ***
herb              2.889e-04  3.908e-04   0.739 0.460146    
mat_tr            3.851e-03  1.310e-03   2.939 0.003479 ** 
shrb              7.502e-03  5.419e-03   1.384 0.167059    
herb:mat_tr      -3.530e-05  1.040e-05  -3.393 0.000759 ***
herb:shrb        -1.662e-05  2.339e-05  -0.710 0.477830    
mat_tr:shrb      -9.927e-05  4.992e-05  -1.989 0.047428 *  
herb:mat_tr:shrb  1.015e-06  3.732e-07   2.720 0.006820 ** 
---
...
```{r}
ggplot(habitat_structure_predictors_insect_pred, aes(x = herb, y = insect_richness, color = factor(mat_tr_bin))) +
  geom_smooth(size = 1) +
  labs(title = "Interaction effect of herb and shrub on insect_richness",
       x = "herb",
...
habitat_structure_predictors_insect_pred2 <- expand.grid(
  ...
  herb = seq(min(habitat_structure_predictors$herb), max(habitat_structure_predictors$herb), length.out = 50)
)

habitat_structure_predictors_insect_pred2 = habitat_structure_predictors_insect_pred2 %>%
  filter(mat_tr + shrb + herb <= 400)
...
p1 = ggplot(habitat_structure_predictors_insect_pred2, aes(x = mat_tr, y = herb, fill = insect_richness)) +
  geom_tile() +
...
p2 = ggplot(habitat_structure_predictors_insect_pred2, aes(x = shrb, y = herb, fill = insect_richness)) +
  geom_tile() +
  scale_fill_viridis_c() +  # Better color scaling
  labs(
       x = "shrb", 
...
title <- ggdraw() + 
  draw_label(
    "Interaction effects of herb, shrb and mat_tree on insect richness",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
...
```{r}
ggplot(habitat_structure_predictors_plant_pred, aes(x = herb, y = plant_richness, color = factor(shrb_bin))) +
  geom_smooth(size = 1) +
  labs(title = "Interaction effect of herb and shrub on plant richness",
       x = "herb",
...
ggplot(habitat_structure_predictors_plant_pred2, aes(x = herb, y = shrb, fill = plant_richness)) +
  geom_tile() +
  scale_fill_viridis_c() +  # Better color scaling
  labs(title = "Interaction effect of herb and shrb on plant richness",
       x = "herb", 
       y = "shrb", 
